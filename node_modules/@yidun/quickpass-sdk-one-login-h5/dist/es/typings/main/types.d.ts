...
export interface PhoneStyles extends Omit<CommonStyles, 'width'> {
    ...
    subWidth?: number | `${number}px`;
    subHeight?: number | `${number}px`;
    squareBackground?: string;
    squareWidth?: number | `${number}px`;
    squareHeight?: number | `${number}px`;
    squareRadius?: number | `${number}px`;
}
export type PhoneInputStyle = 'sub' | 'square';
export interface SwitchButtonStyles extends CommonStyles {
    color?: string;
    size?: number | `${number}px`;
}
export type PolicyTarget = typeof POLICY_TARGET[keyof typeof POLICY_TARGET];
...
export type Mode = 'float' | 'popup';
export type Element = 'back' | 'logo' | 'slogan' | 'phone' | 'switchButton' | 'policy' | 'loginButton';
